(() => {
  ...
  const canEditDates = state.canEditDates !== false;
  ...
  function applyTotalsSnapshot(totals) {
    ...
    setText('balance', totals.remaining);
    setValue('paymentTotal', totals.grandTotal);
    setValue('alreadyPaid', totals.paidAmount);
  ...
  function bindFixedActionButtons() {
    ...
    bindClickOnce('saveAndPrint', () => completeCheckIn(true));
    bindClickOnce('undoCheckIn', undoCheckIn);
    bindClickOnce('checkOutAction', checkOut);
    bindClickOnce('openExtend', () => openModal('extendModal'));
    bindClickOnce('printInvoice', () => openLegacyInvoice('CHECK-IN'));
    bindClickOnce('printLaundry', openLaundryInvoice);
    bindClickOnce('postFbr', postFbr);
  }

  async function refreshReservationUi(targetRegId = regId) {
    if (!targetRegId) return;
    const url = new URL(window.location.href);
    url.searchParams.delete('RI');
    url.searchParams.set('q', targetRegId);
    const response = await fetch(url.toString(), {
      credentials: 'same-origin',
      headers: { 'X-Requested-With': 'XMLHttpRequest' }
    });
    if (!response.ok) throw new Error(`Unable to refresh reservation data (${response.status}).`);
    const html = await response.text();
    const doc = new DOMParser().parseFromString(html, 'text/html');

    const stateNode = doc.getElementById('checkinInitialState');
    if (stateNode) {
      try {
        const nextState = JSON.parse(stateNode.textContent || '{}');
        Object.keys(state).forEach(k => delete state[k]);
        Object.assign(state, nextState);
        regId = String(nextState.reservationId || targetRegId || regId).trim();
        visitId = String(nextState.visitId || visitId || '').trim();
        currency = nextState.currency || currency;
        currencyCode = String(nextState.currencyCode || currencyCode || 'GBP').toLowerCase();
        app.dataset.regId = regId;
        app.dataset.visitId = visitId;
        app.dataset.currency = currency;
        app.dataset.currencyCode = String(nextState.currencyCode || '').toUpperCase();
      } catch { }
    }

    ['chargeRows', 'paymentLogRows', 'securityRows', 'laundryRows', 'discountList'].forEach(id => replaceInnerFromDocument(doc, id));
    ['subTotal', 'taxTotal', 'grandTotal', 'balance', 'depositDisplay'].forEach(id => {
      const current = byId(id), fresh = doc.getElementById(id);
      if (current && fresh) current.innerHTML = fresh.innerHTML;
    });
    ['paymentTotal', 'advancePaid', 'roomSecurity', 'alreadyPaid', 'payableAmount', 'checkIn', 'checkOut'].forEach(id => replaceValueFromDocument(doc, id));

    // Refresh source/company options without disturbing the rest of the form.
    ['source', 'company'].forEach(id => {
      const current = byId(id), fresh = doc.getElementById(id);
      if (!current || !fresh) return;
      const selected = fresh.value || current.value;
      current.innerHTML = fresh.innerHTML;
      setSelectValue(id, selected);
    });

    const currentGuestActions = byId('guestActionRow');
    const freshGuestActions = doc.getElementById('guestActionRow');
    if (currentGuestActions && freshGuestActions) currentGuestActions.innerHTML = freshGuestActions.innerHTML;

    const currentFixed = byId('checkinFixedActions');
    const freshFixed = doc.getElementById('checkinFixedActions');
    if (currentFixed && freshFixed) currentFixed.innerHTML = freshFixed.innerHTML;

    $$('.money-input').forEach(x => x.dataset.currency = currency);
    updateStayCount();
  ...
  function selectCalendarDate(value) {
    ...
    // If the second click is on/before arrival, treat it as a new arrival
    // rather than leaving the picker in an error state.
    if (selected <= arr) {
      byId('checkIn').value = value;
      byId('checkOut').value = '';
      calendarStage = 1;
  ...
  async function updateGuest() {
    ...
    try {
      const result = await api(urls.updateGuest, { method: 'POST', body: { guest: guestPayload() } });
      if (!result.ok) return message(result.message, 'error');
      lastQuickNameSnapshot = currentNameSnapshot();
      message(result.message, 'success');
      // Normal guest-detail changes already exist in the current controls, so do not
      // fetch/rebuild the page. Only a stay-date change needs totals/rows reloaded.
      if (result.data?.datesChanged || result.data?.dateChangeIgnored) {
        if (result.data?.arrival) byId('checkIn').value = result.data.arrival;
        if (result.data?.departure) byId('checkOut').value = result.data.departure;
        updateStayCount();
      }
      if (result.data?.datesChanged) await refreshReservationUi(regId);
    } catch (e) { message(e.message, 'error'); }
    finally { hideBusy(); }
  }
  ...
  async function openRoomChangeForRow(row, paymentId) {
    ...
    try {
      ...
      const select = byId('roomChangeSelect');
      select.innerHTML = '<option value="">--Select available room--</option>';
      (Array.isArray(items) ? items : []).forEach(x => select.insertAdjacentHTML('beforeend', `<option value="${esc(x.value)}">${esc(x.text)}${x.meta ? ` (${esc(x.meta)})` : ''}</option>`));
      byId('roomChangePaymentId').value = String(paymentId);
      byId('roomChangeHint').textContent = `Current room: ${row.dataset.room || '—'} · ${row.dataset.arrival || ''} → ${row.dataset.departure || ''}`;
  ...
  async function saveRoomChange() {
    ...
    const newRoomNo = byId('roomChangeSelect')?.value || '';
    if (!paymentId || !newRoomNo) return message('Select the new room.', 'error');
  ...
  async function saveSecurity() {
    ...
    const method = byId('securityMethod')?.value || 'Cash';
    if (amount <= 0) return message('Enter a security amount greater than 0.', 'error');
    if (!note) return message('Additional security information is required.', 'error');

    if (method.toLowerCase().includes('card') && app.dataset.stripe === '1' && movement === 'deposit') {
      byId('cardAmount').value = amount.toFixed(2);
      byId('cardNote').value = note;
      if (byId('cardReader') && byId('securityReader')?.value) byId('cardReader').value = byId('securityReader').value;
    ...
    showBusy('Posting room security…');
    try {
      const r = await api(urls.securityMovement, { method: 'POST', body: { regId, visitId, amount, note, method, movement, securityId: pendingSecurityId, paymentIntentId: '' } });
      if (!r.ok) return message(r.message, 'error');
      pendingSecurityId = 0;
      closeModal('securityModal'); message(r.message, 'success'); await refreshReservationUi(regId);
    } catch (e) { message(e.message, 'error'); } finally { hideBusy(); }
  }

  async function settleSecurity(id) {
    const row = document.querySelector(`tr[data-security-id="${id}"]`);
    const amount = toNumber(row?.dataset.securityAmount);
    pendingSecurityId = id;
    byId('securityMovement').value = 'refund';
    byId('securityAmount').value = amount > 0 ? amount.toFixed(2) : '';
    byId('securityNote').value = `Settlement for security row ${id}`;
    const pi = row?.dataset.securityPi || '';
    if (pi && byId('securityMethod')) byId('securityMethod').value = 'Card';
  ...
  async function completeManualStripePayment(result, amount, note) {
    ...
    const save = await api(urls.recordPayment, {
      ...
      body: {
        ...
        method: 'Manual Card - Stripe',
        roomSecurity: 0,
        note: note || 'Stripe Checkout manual card payment',
    ...
    message('Manual card payment recorded successfully.', 'success');
    closeModal('cardModal');
    await refreshReservationUi(regId);
  }

  async function startManualStripeCheckout(amount, note) {
    ...
    try {
      created = await api(urls.stripeCheckout, {
        method: 'POST',
        body: { regId, visitId, amount, currency: currencyCode, note }
      });
    } catch (e) {
    ...
    manualCheckoutPoll = setInterval(async () => {
      ...
      try {
        ...
        if (result?.success && (status === 'succeeded' || status === 'paid' || status === 'complete')) {
          await completeManualStripePayment(result, amount, note);
          return;
        }
  ...
  byId('paidAmount')?.addEventListener('input', () => {
    ...
    byId('balance').textContent = money(Math.max(0, total - already - toNumber(byId('paidAmount').value)));
  });
  bindFixedActionButtons();
  byId('extendSave')?.addEventListener('click', extendReservation);
  ...
  byId('openRoomSecurity')?.addEventListener('click', () => openModal('securityModal'));
  ...
  byId('openCardPayment')?.addEventListener('click', () => openPdqPaymentLikeWebForms());
  ...
  $$('.modal-backdrop').forEach(modal => modal.addEventListener('click', e => { if (e.target === modal && modal.id !== 'confirmDialog') closeModal(modal.id); }));
